...
$(document).ready(function() {
    ...
    let savedTerm = $('.saved-term').val();
    //deprecate the usage of global variables
    ...
    function handleSearch() {
        $('.search-form').on('submit', function(event) {
            ...
            //use event.currentTarget to specify the value of the current event
            const searchStringParent = $(event.currentTarget).find('.query');
            const searchString = searchStringParent.val();

            $('saved-term').val(searchString);
            // clear and deselect input
            //$('.query').val('').blur();
            searchStringParent.val('');
    ...
    function handleAPIResponse(response) {
        ...
        let prevPageToken = response.prevPageToken;

        handleNextPage(nextPageToken);
        handlePrevPage(prevPageToken);
    }
    ...
    function handleNextPage(nextPageToken) {
        $('.next-results').on('click', function() {
            getAPIData(savedTerm, nextPageToken, handleAPIResponse);
        })
    }

    // listens for a click on the Prev page button
    // calls API to request the Prev page of data
    function handlePrevPage(prevPageToken) {
        $('.prev-results').on('click', function() {
            getAPIData(savedTerm, prevPageToken, handleAPIResponse);
        })
    }
